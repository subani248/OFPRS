...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    if (origin.indexOf('vercel.app') !== -1 || origin.indexOf('onrender.com') !== -1) {
      return callback(null, true);
    }
...
// Root route - either serve frontend or API message
app.get('/', (req, res) => {
  if (process.env.NODE_ENV === 'production') {
    // In production, this will be handled by the frontend serving code below
    res.json({ message: 'OFPRS API is running...' });
  } else {
    res.json({ message: 'OFPRS API is running... (Development mode)' });
  }
});
...
if (process.env.NODE_ENV === 'production') {
  ...
  if (fs.existsSync(frontendBuildPath)) {
    ...
    app.get('/', (req, res) => {
      res.json({ message: 'OFPRS API is running... (Frontend build not found)' });
    });
  }
}
